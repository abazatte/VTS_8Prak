...
public class BillBoard {
    ...
    public String convertToJson(String ownerIp){
        StringBuilder sb = new StringBuilder();
        sb.append("{").append("\"entries\":[");
        for(BillBoardEntry entry: this.billboard){
            sb.append(entry.convertToJson());
            if(entry != this.billboard[this.billboard.length - 1]){
                sb.append(",");
            }
        }
        sb.append("]").append(",\"owner\": \"").append(ownerIp).append("\"}");
        return sb.toString();
    }
}
